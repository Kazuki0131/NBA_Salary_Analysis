...
L.tileLayer("https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token={accessToken}", {
  ...
  maxZoom: 18,
  id: "mapbox.streets",
  accessToken: decode_data(KEY, CODE)
}).addTo(myMap);

// AWS S3 bucket
//replace the url below with your s3 bucket url
...
function teamMarkers(team_data, team_players, teams_wins,teams_data){
  ...
  icon_url = aws_s3_bucket + '/static/images/'+team_data.team_abbr+'_logo.png';
...
// decode the key
function decode_data(key, code){
  string_position = parseInt(code.substring(1,2));
  string_length = parseInt(code.substring(0,1));
  // console.log("string_p: ", string_position);
  // console.log("string.l: ", string_length);
  k_length = key.length;
  start_p = string_position + string_length
  a_key = key.substring(0,string_position) + key.substring(start_p, k_length);
  // console.log(a_key)
  // console.log(API_KEY)
  return a_key;
}
...
function plotRadarCahrt(event){
  ...
  Object.entries(team_data).forEach(([key, value]) => {
    if(key === "rk_AST" || key === "rk_BLK" || key === "rk_PTS" || key === "rk_STL" || key === "rk_TOV" || key === "rk_TRB"){
      ...
      if(index === 0){
        first_label = labels[key];
        first_status = Math.round((30-value)/29*100);
      }
      index += 1;
    }
  });
  // convert the status numbers from rankings(1 - 30) to power scores(0 - 100)
  // team_status.push(Math.round((30-team_data.rk_AST)/29*100));
  team_status.push(first_status);
  status_labels.push(first_label)
    ...
    xAxis: {
      ...
      labels: {
        ...
        formatter: function () {
          ...
          // return '<span><img src="/static/images/'+ output + '" style="width: 40px; height: 40px;"/><br></span>';
          return '<span><img src="' + aws_s3_bucket + '/static/images/'+ output + '" style="width: 40px; height: 40px;"/><br></span>';
        }
      }
    },
    yAxis: [{
...
// teams_data_url = "http://127.0.0.1:5000/teams_json"
// players_data_url = "http://127.0.0.1:5000/players_json"

teams_data_url = "https://s1ia6rnpx4.execute-api.us-east-1.amazonaws.com/dev/teams_json"
players_data_url = "https://s1ia6rnpx4.execute-api.us-east-1.amazonaws.com/dev/players_json"
